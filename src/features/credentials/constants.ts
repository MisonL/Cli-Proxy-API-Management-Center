...
export type CredentialModelItem = {
  id: string;
  display_name?: string;
  type?: string;
  owned_by?: string;
...
export const getCredentialStableKey = (
  file: Pick<CredentialItem, 'id' | 'runtimeId' | 'name'>
): string => {
  if (typeof file.id === 'string' && file.id.trim()) {
    return file.id.trim();
  }
  if (typeof file.runtimeId === 'string' && file.runtimeId.trim()) {
    return file.runtimeId.trim();
  }
  return String(file.name ?? '').trim();
};

export const getCredentialStatusMessage = (file: CredentialItem): string => {
  const raw = file.statusMessage;
  if (typeof raw === 'string') return raw.trim();
  if (raw == null) return '';
  return String(raw).trim();
};

export const hasCredentialStatusMessage = (file: CredentialItem): boolean =>
  getCredentialStatusMessage(file).length > 0;
...
export function resolveCredentialStats(file: CredentialItem, stats: KeyStats): KeyStatBucket {
  if (
  ...
  if (selectionKeyKey && stats.bySelectionKey?.[selectionKeyKey]) {
    return stats.bySelectionKey[selectionKeyKey];
  }
...
export const formatModified = (item: CredentialItem): string => {
