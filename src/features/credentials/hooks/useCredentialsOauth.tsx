...
import type { CredentialItem, OAuthModelAliasEntry } from '@/types';
import type { CredentialModelItem } from '@/features/credentials/constants';
import { normalizeProviderKey } from '@/features/credentials/constants';

type UnsupportedError = 'unsupported' | null;
type ViewMode = 'diagram' | 'list';

export type UseCredentialsOauthResult = {
...
export type UseCredentialsOauthOptions = {
  viewMode: ViewMode;
  files: CredentialItem[];
};

export function useCredentialsOauth(options: UseCredentialsOauthOptions): UseCredentialsOauthResult {
  ...
  const [allProviderModels, setAllProviderModels] = useState<Record<string, CredentialModelItem[]>>(
    {}
  );
    ...
    const loadAllModels = async () => {
      ...
      const results = await Promise.all(
        providerList.map(async (provider) => {
          try {
            const models = await credentialsApi.getModelDefinitions(provider);
            return { provider, models };
          } catch {
            return { provider, models: [] as CredentialModelItem[] };
          }
        })
      );
      ...
      const nextModels: Record<string, CredentialModelItem[]> = {};
  ...
  const loadExcluded = useCallback(async () => {
    try {
      const res = await credentialsApi.getOauthExcludedModels();
  ...
  const loadModelAlias = useCallback(async () => {
    try {
      const res = await credentialsApi.getOauthModelAlias();
    ...
    (provider: string) => {
      ...
      showConfirmation({
        ...
        onConfirm: async () => {
          ...
          try {
            await credentialsApi.deleteOauthExcludedEntry(providerKey);
            await loadExcluded();
            showNotification(t('oauth_excluded.delete_success'), 'success');
          } catch (err: unknown) {
            try {
              const current = await credentialsApi.getOauthExcludedModels();
              const next: Record<string, string[]> = {};
              Object.entries(current).forEach(([key, models]) => {
                if (normalizeProviderKey(key) === providerKey) return;
                next[key] = models;
              });
              await credentialsApi.replaceOauthExcludedModels(next);
              await loadExcluded();
              showNotification(t('oauth_excluded.delete_success'), 'success');
  ...
  const deleteModelAlias = useCallback(
    (provider: string) => {
      showConfirmation({
        ...
        onConfirm: async () => {
          try {
            await credentialsApi.deleteOauthModelAlias(provider);
  ...
  const handleMappingUpdate = useCallback(
    async (provider: string, sourceModel: string, newAlias: string) => {
      ...
      try {
        await credentialsApi.saveOauthModelAlias(normalizedProvider, nextMappings);
        await loadModelAlias();
        ...
        onConfirm: async () => {
          ...
          try {
            if (nextMappings.length === 0) {
              await credentialsApi.deleteOauthModelAlias(normalizedProvider);
            } else {
              await credentialsApi.saveOauthModelAlias(normalizedProvider, nextMappings);
            }
            await loadModelAlias();
      ...
      try {
        const results = await Promise.allSettled(
          providersToUpdate.map(([provider, mappings]) => {
            ...
            return credentialsApi.saveOauthModelAlias(provider, nextMappings);
          })
        );
          ...
          try {
            const results = await Promise.allSettled(
              providersToUpdate.map(([provider, mappings]) => {
                ...
                if (nextMappings.length === 0) {
                  return credentialsApi.deleteOauthModelAlias(provider);
                }
                return credentialsApi.saveOauthModelAlias(provider, nextMappings);
              })
            );
