...
import { CredentialsActionResultModal } from './CredentialsActionResultModal';

vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string, options?: Record<string, unknown>) => {
      const translations: Record<string, string> = {
        'credentials.batch_delete_result_title': '批量删除结果',
        'notification.delete_failed': '删除失败',
        'common.close': '关闭',
      };
      if (key === 'credentials.batch_result_summary') {
        return `总计 ${options?.total ?? 0} 个，成功 ${options?.success ?? 0} 个`;
      }
      if (key === 'credentials.batch_delete_partial') {
...
describe('CredentialsActionResultModal', () => {
  it('展示动作失败明细并支持关闭', () => {
