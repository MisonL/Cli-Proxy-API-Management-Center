...
import styles from './CredentialsOAuthExcludedEditPage.module.scss';

type CredentialModelItem = { id: string; display_name?: string; type?: string; owned_by?: string };

type LocationState = { fromCredentials?: boolean } | null;
...
export function CredentialsOAuthExcludedEditPage() {
    ...
    const load = async () => {
      ...
      try {
        const [filesResult, excludedResult, aliasResult] = await Promise.allSettled([
          credentialsApi.list(),
          credentialsApi.getOauthExcludedModels(),
          credentialsApi.getOauthModelAlias(),
        ]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      if (models.length) {
        await credentialsApi.saveOauthExcludedModels(normalizedProvider, models);
      } else {
        await credentialsApi.deleteOauthExcludedEntry(normalizedProvider);
      }
