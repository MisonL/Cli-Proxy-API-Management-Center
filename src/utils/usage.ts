...
export interface KeyStats {
  ...
  bySelectionKey: Record<string, KeyStatBucket>;
}
...
export interface UsageDetail {
  timestamp: string;
  source: string;
  selection_key: number | string | null;
  tokens: {
    input_tokens: number;
...
const resolveDetailSelectionKey = (
  detail: Pick<UsageDetail, 'selection_key'> | Record<string, unknown> | null | undefined
) => {
  if (!detail || typeof detail !== 'object') return null;
  const record = detail as Record<string, unknown>;
  return normalizeAuthIndex(record.selection_key ?? record.auth_index);
};
    ...
    Object.entries(models).forEach(([modelName, modelEntry]) => {
      ...
      modelDetails.forEach((detailRaw) => {
        ...
        details.push({
          ...
          selection_key: (detailRaw.selection_key ??
            detailRaw.auth_index) as UsageDetail['selection_key'],
...
export function calculateStatusBarData(
  ...
  usageDetails.forEach((detail) => {
    ...
    if (
      selectionKeyFilter !== undefined &&
      resolveDetailSelectionKey(detail) !== String(selectionKeyFilter)
    ) {
      return;
    }
...
export function computeKeyStats(
  ...
  const apis = getApisRecord(usageData);
  if (!apis) {
    return { bySource: {}, bySelectionKey: {} };
  }

  const sourceStats: Record<string, KeyStatBucket> = {};
  const selectionKeyStats: Record<string, KeyStatBucket> = {};
  ...
  Object.values(apis).forEach((apiEntry) => {
    ...
    Object.values(models).forEach((modelEntry) => {
      ...
      details.forEach((detail) => {
        ...
        const selectionKeyKey = resolveDetailSelectionKey(detailRecord);
        ...
        if (selectionKeyKey) {
          const bucket = ensureBucket(selectionKeyStats, selectionKeyKey);
          if (isFailed) {
            bucket.failure += 1;
          } else {
            bucket.success += 1;
...
export function computeKeyStatsFromDetails(usageDetails: UsageDetail[]): KeyStats {
  ...
  usageDetails.forEach((detail) => {
    ...
    const selectionKeyKey = resolveDetailSelectionKey(detail);
    ...
    if (selectionKeyKey) {
      const bucket = ensureBucket(bySelectionKey, selectionKeyKey);
      if (isFailed) {
        bucket.failure += 1;
      } else {
        bucket.success += 1;
